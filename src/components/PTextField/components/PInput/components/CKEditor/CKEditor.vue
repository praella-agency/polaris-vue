...
<script setup>
    ...
    const emit = defineEmits(['destroy', 'update:modelValue', 'input', 'ready']);

    let ckEditor = ref(null);
    let instance = ref(null);
    let lastEditorData = ref({
        type: String,
        default: ''
    });
    ...
    onBeforeUnmount(() => {
        ...
        emit('destroy', instance.value);
    });

    onMounted(() => {
        ckEditor = ckEditor.value;
        ...
        const editorConfig = Object.assign({}, props.config);

        if (props.modelValue) {
            editorConfig.initialData = props.modelValue;
        }

        editor.value.create(ckEditor, editorConfig)
            .then(editor => {
                // Save the reference to the instance for further use.
                instance = vue.markRaw(editor);
    ...
    watch(() => props.modelValue, (oldValue, newValue) => {
        ...
        //
        // See: https://github.com/ckeditor/ckeditor5-vue/issues/42.
        if (newValue !== oldValue && newValue !== lastEditorData) {
            instance.value.setData(newValue);
        }
    });

    watch(() => props.disabled,(readOnlyMode) => {
        if (readOnlyMode) {
            instance.value.enableReadOnlyMode(SAMPLE_READ_ONLY_LOCK_ID);
        } else {
            instance.value.disableReadOnlyMode(SAMPLE_READ_ONLY_LOCK_ID);
        }
    });
</script>
